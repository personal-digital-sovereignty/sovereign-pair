...
- **Glassmorphism Download Overlay (UI)**: Desenvolvido um interceptador Regex no parser estático do `ChatPanel.svelte` que detecta tags `<img>` oriundas de Markdown. A imagem é dinamicamente injetada dentro de uma moldura arredondada com um botão translúcido estético assíncrono (Hover), permitindo que o usuário capture a pintura (`download="Sovereign_Artefact.png"`) nativamente no File System do navegador com 1 clique.
- **Tool-Calling Resilience (Anti-Crash 400)**: Erradicada a falha onde modelos brutos de raciocínio lógico que desconhecem Schemas JSON (ex: `gemma3`) abortavam o SSE e matavam a UI principal através do `400 Bad Request`. Injetado um Loop Autônomo e Curativo (`api.rs`) que intercepta o choque do Ollama, purifica (stript) o payload extraindo violentamente o contrato das Tools, e dispara uma nova tentativa no mesmo pentésimo de segundo. A requisição flui lindamente e ininterruptamente se moldando para o Plain-Text tradicional.
...
- **Sovereign Manifesto (Menos é Mais)**: Refatoração estrutural da documentação. Consolidamos 24 antigos artefatos obsoletos (12 em PT-BR e 12 em EN-US) em um único e definitivo manifesto corporativo (`SOVEREIGN_MANIFESTO.md`). O texto foi "humanizado", expurgando jargões excessivos e registrando oficialmente a era do Motor Cíbrido (Rust + Svelte + Nodes CLI), sepultando as dependências antigas do LlamaIndex e ChromaDB.
