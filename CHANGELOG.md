...
### Changed
- **Unsloth Monitor & Reflection Lab (Architectural Mocks)**: O painel visual do Unsloth Monitor e do Reflection Lab foram projetados e documentados nesta versão (1.2.0) como *Mocks de Alta Fidelidade*. Eles interagem com funções assíncronas do motor Rust (como disparos de intervalos controlados de "Epochs" no background e renderização de SSE Logs), porém não manipulam tensores. Foram deixados implementados para pavimentar a Interface visual complexa da v1.3.0 sem arrebentar a malha de VRAM atual. O *Unsloth Monitor* teve suas strings `Llama-3-8B` hardcoded substituídas para puxar ativamente o nome dos modelos dinamicamente a partir da malha local, respeitando a integração do SQLite, mas a perda/gradiente é cenográfica.
