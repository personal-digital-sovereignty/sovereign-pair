...
### 1.1 The Standalone Binary-Compose Symphony

The entire stack is containerized. No host dependencies are required beyond Standalone Binary and a mesh VPN.

#### Containerized Components

1.  **Rust Axum Core Gateway (`sovereign-api`):** The beating heart. Constructed completely natively compiling C/C++ elements linked via **Rust (Axum + Tokio)**. It orchestrates NLP structural queries over standard vectors natively via an internal `sqlite-vec` index mechanism avoiding obsolete Rust processing bottlenecks.
2.  **Sensus Vault Client Engine UI (`sovereign-web`):** Static native component frontend mapped under Svelte 5 logic frameworks preventing external API bindings processing outputs entirely client-side.
...
> *Standalone Binary* is just a magical box. Instead of installing Rust, Postgres, Redis, and dozens of libraries on your PC (and inevitably breaking an update), Standalone Binary runs a mini-computer for each service safely inside a locked box. To start the entire infrastructure, you only type `sovereign-core up -d`.

> [!NOTE] 🧬 **Living Code: Container Orchestration (SHA: `94bfb2f`)**
> ▫️ **N8N Cloud Topology:** `sovereign-core.n8n.yml`
> ▫️ **Local Backend Topology:** `sovereign-core.yml`
> ▫️ **Containers (Standalone Binaryfiles):** `Standalone Binaryfile.api` & `Standalone Binaryfile.web`
...
4. In your VPS `sovereign-core.yml`, bind the exposed ports strictly to the Tailnet IP.
...
- **Role:** The Zero-Trust Fortress. Fiercely guards your PDFs ("Sensus Vault"), ChromaDB Vector index, and executes standard HTTP/RAG logic (Axum, N8N).
...
> ▫️ **HTTP Backbone:** `src/api/main.py` (Axum)
