...
| **Orchestrator** | Linux Infra | O Servidor em Nuvem (Ex: Oracle OCI Grátis) onde roda a "Lógica Python" de roteamento cognitivo (Banco de Dados Relacionais, N8N, e a API FastAPI dos agentes). Ele gerencia a conexão com o usuário final via HTTP/VueJS mas transfere o pesado fardo de inferência bruta de texto para o nó externo. |
| **Inference Node** | Linux Infra | O hardware físico potente (Ex: Laptops Ryzen ou Desktops com placa gráfica isolados numa VPN) assumindo toda a aceleração gráfica pesada e bruta (Múltiplos `Ollama/GGUFs`). Ele funciona puramente como operário `VRAM` terceirizado para a Nuvem Oracle. |
...
| **Cíbrido (Cybrid Topology)** | Redes | A arquitetura híbrida de hospedar os serviços de Orquestração Web em uma Nuvem pública de baixo custo, conectando-os de forma altamente segura através de túneis encriptados até um servidor físico local com aceleração térmica (GPU/Ryzen). Isso garante altíssima performance processual da nuvem sem precisar expor os Bancos de Dados locais ou Computação doméstica à Internet Aberta, unindo-os via protocolo de malha `Tailscale`. |
