...
- **A Operação:** 
   - *Se você for pobre (8GB a 12GB VRAM Ex: RTX 3060/4060):* Coloque os pés no chão. Cancele modelos titânicos e adote modelos pequenos, geniais e com lobotomia artificial controlada, chamados de Modelos Quantizados, como o excelente e brutal chinês `qwen2.5:0.5b` (Roda em celular de tia), `llama3.2:1b` ou os eficientes da Mistral/Google (Gemma).
   - *Se a sua Corporação tiver dinheiro (24GB a 48GB VRAM Ex: RTX 3090/4090 ou MAC Studios M2 Max Ultra de 128GB unificado):* Você virou um Deus e rodará modelos insanos e lentos que parecem humanos (Ex: O monstro da Meta, `llama-3-70b-instruct`) e o RAG nunca precisará compactar tanto o texto pra não quebrar a sanidade antes de fechar a thread.

> [!NOTE]
> Regra de ouro: Arquiteturas de Orquestração costumam derrubar chamadas de webhooks que demoram após 2 minutos de forma genérica para evitar sobrecarga de memória do Windows (o famoso Axios Timeout). Assim... se o Servidor da Nuvem chamar seu PC Gamer velhinho, e ele demorar 4 minutos matutando pra cuspir o Parágrafo do OLLAMA e enviar, a Nuvem já mandou seu PC pastar. Para contornar e resolver este lixo Corporativo crônico, nós injetamos um limite superior elástico brutal e passivo no back-end (Variável `REQUEST_TIMEOUT="300.0"`). Isso impõe e obriga o servidor Nuvem do N8N a ficar parado esperando o seu Llama por estonteantes eternos gloriosos 5 minutos sem travar e sem retornar telas brancas com `500 Internal Server Error`. Respire o atraso físico da máquina. Abrace o Local-First.
