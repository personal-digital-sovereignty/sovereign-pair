<template>
  <div class="h-full overflow-hidden w-full bg-surface-900 text-surface-200 flex flex-col">
    <header class="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-surface-700 bg-surface-900/80 backdrop-blur z-10">
      <div>
        <h1 class="text-2xl font-light tracking-tight flex items-center gap-3 text-surface-100">
      ...
      <Teleport to="#sidebar-context-area" v-if="isMounted">
        <div class="p-3 w-full h-full flex flex-col gap-2 mt-4">
          <div class="px-2 py-1.5 rounded-lg cursor-pointer transition-all flex items-center gap-2"
               ...
               :class="activeTab === 'overview' ? 'bg-surface-800 border-l border-surface-700 shadow-sm shadow-emerald-500/10 text-emerald-400' : 'text-surface-500 hover:text-surface-200 transition-colors'">
            <i class="i-lucide-layout-dashboard text-sm"></i>
            <span class="text-sm font-medium">Overview</span>
          </div>
          <div class="px-2 py-1.5 rounded-lg cursor-pointer transition-all flex items-center gap-2"
               @click="activeTab = 'graph'"
               :class="activeTab === 'graph' ? 'bg-surface-800 border-l border-surface-700 shadow-sm shadow-emerald-500/10 text-emerald-400' : 'text-surface-500 hover:text-surface-200 transition-colors'">
          ...
          <div class="px-2 py-1.5 rounded-lg cursor-pointer transition-all flex items-center gap-2 mt-4"
               ...
               :class="activeTab === 'blue-collar' ? 'bg-surface-800 border-l border-surface-700 shadow-sm shadow-orange-500/10 text-orange-400' : 'text-surface-500 hover:text-orange-400 transition-colors'">
    ...
    <main class="flex-1 overflow-hidden relative">
        <div class="absolute inset-0 w-full h-full overflow-y-auto p-6 bg-surface-900/50 custom-scroll">
            <template v-if="activeTab === 'overview'">
              <div class="w-full flex flex-col gap-6 relative min-h-[calc(100vh-140px)]">
                  ...
                  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 shrink-0 min-h-[220px]">
                      <div class="bg-surface-800 border border-surface-700/50 rounded-xl overflow-hidden relative p-4 flex flex-col justify-center shadow-xl">
                          <TokenMetricsTracker />
                      </div>
                  ...
                  <div class="flex-1 min-h-[300px] h-full bg-surface-800 border border-surface-700/50 rounded-xl overflow-hidden flex flex-col p-4 custom-scroll shadow-xl">
                      <RealtimeLogs />
                  </div>
              </div>
            </template>
            <template v-else-if="activeTab === 'graph'">
              <div class="w-full h-[calc(100vh-140px)] bg-surface-800 border border-surface-700/50 rounded-xl overflow-hidden shadow-xl">
                 <CognitiveGraph />
              </div>
            </template>
