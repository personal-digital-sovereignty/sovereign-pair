                ...
                Ok(blueprint) => {
                    ...
                    for (i, step) in blueprint.plan.iter().enumerate() {
                        ...
                        let executor_payload = json!({
                            "model": "qwen2.5:3b",
                            "messages": [
                                { "role": "system", "content": "Você é o Agente Executor. Realize a ação informada perfeitamente. Responda apenas com o resultado da ação e NADA de blá-blá-blá." },
                                { "role": "user", "content": format!("Contexto Anterior: {}\n\nAção a tomar agora: ({}) -> {}", aggregated_results, step.action, step.task) }
                            ],
                            "stream": false
                        });
                        ...
                        if let Ok(exec_res) = client.post(ollama_url).json(&executor_payload).send().await {
                            if let Ok(exec_json) = exec_res.json::<serde_json::Value>().await {
                                if let Some(exec_msg) = exec_json.get("message").and_then(|m| m.get("content").and_then(|c| c.as_str())) {
