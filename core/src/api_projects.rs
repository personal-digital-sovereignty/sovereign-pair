...
pub async fn get_projects_handler(
    ...
    let rows = sqlx::query_as::<_, ProjectRow>(
        "SELECT id, tenant_id, name, purpose, traction_status, next_action, energy_level, progress_percent, friction_radar, deadline, is_archived, columns_json, created_at FROM projects WHERE tenant_id = ? ORDER BY created_at DESC"
    )
    .bind(&tenant)
    .fetch_all(&state.db)
    .await;
...
pub async fn delete_project_handler(
    ...
    let tenant = extract_tenant(&headers);
    let _ = sqlx::query("DELETE FROM projects WHERE id = ? AND tenant_id = ?").bind(id).bind(tenant).execute(&state.db).await;
...
pub async fn update_project_handler(
    ...
    let tenant = extract_tenant(&headers);
    let old_proj = sqlx::query_as::<_, ProjectRow>("SELECT id, tenant_id, name, purpose, traction_status, next_action, energy_level, progress_percent, friction_radar, deadline, is_archived, columns_json, created_at FROM projects WHERE id = ? AND tenant_id = ?")
        .bind(&id)
        .bind(&tenant)
    ...
    if let Ok(Some(proj)) = old_proj {
        ...
        let _ = sqlx::query(
            "UPDATE projects SET name = ?, purpose = ?, traction_status = ?, next_action = ?, energy_level = ?, progress_percent = ?, friction_radar = ?, deadline = ?, is_archived = ?, columns_json = ? WHERE id = ? AND tenant_id = ?"
        )
        .bind(&final_name)
        .bind(&final_purpose)
        .bind(&final_traction)
    ...
    (axum::http::StatusCode::NOT_FOUND, Json(serde_json::json!({"error": true, "message": "Project not found or unauthorized access."}))).into_response()
}
...
pub async fn get_project_documents_handler(
    ...
    let tenant = extract_tenant(&headers);
    // Verificar se o projeto pertence ao tenant antes de expor os documentos
    let proj_check = sqlx::query("SELECT id FROM projects WHERE id = ? AND tenant_id = ?").bind(&project_id).bind(&tenant).fetch_optional(&state.db).await;
    
    if let Ok(Some(_)) = proj_check {
        ...
        match rows {
            Ok(docs) => return Json(docs).into_response(),
            Err(e) => tracing::error!("SQLx Error lendo Project Documents: {}", e),
        }
    }
    Json(serde_json::json!([])).into_response()
}
...
pub async fn link_project_document_handler(
    ...
    if let Ok(Some(_)) = proj_check {
        let _ = sqlx::query("INSERT OR IGNORE INTO project_documents (project_id, file_path) VALUES (?, ?)")
            ...
            .execute(&state.db)
            .await;
        return Json(serde_json::json!({"status": "linked", "file_path": payload.file_path})).into_response()
    }
    (axum::http::StatusCode::UNAUTHORIZED, Json(serde_json::json!({"error": "Unauthorized."}))).into_response()
}
...
pub async fn unlink_project_document_handler(
    ...
    if let Ok(Some(_)) = proj_check {
        ...
        let _ = sqlx::query("DELETE FROM project_documents WHERE project_id = ? AND file_path = ?")
            ...
            .execute(&state.db)
            .await;
        return Json(serde_json::json!({"status": "unlinked"})).into_response()
    }
    (axum::http::StatusCode::UNAUTHORIZED, Json(serde_json::json!({"error": "Unauthorized."}))).into_response()
}
...
pub async fn get_project_tasks_handler(
    ...
    let rows = sqlx::query_as::<_, TaskRow>(
        "SELECT id, project_id, tenant_id, title, description, status, priority, order_index, deadline, created_at FROM tasks WHERE project_id = ? AND tenant_id = ? ORDER BY order_index ASC"
    )
    .bind(project_id)
    .bind(tenant)
    .fetch_all(&state.db)
...
pub async fn delete_task_handler(
    ...
    let tenant = extract_tenant(&headers);
    let _ = sqlx::query("DELETE FROM tasks WHERE id = ? AND tenant_id = ?").bind(id).bind(tenant).execute(&state.db).await;
    Json(serde_json::json!({"status": "deleted"})).into_response()
}
...
pub async fn update_task_handler(
    ...
    let old_task = sqlx::query_as::<_, TaskRow>("SELECT id, project_id, tenant_id, title, description, status, priority, order_index, deadline, created_at FROM tasks WHERE id = ? AND tenant_id = ?")
        .bind(&id)
        .bind(&tenant)
        .fetch_optional(&state.db)
        .await;
    ...
    if let Ok(Some(task)) = old_task {
        ...
        let _ = sqlx::query(
            "UPDATE tasks SET title = ?, description = ?, status = ?, priority = ?, order_index = ?, deadline = ? WHERE id = ? AND tenant_id = ?"
        )
        .bind(&final_title)
        .bind(&final_desc)
        .bind(&final_status)
    ...
    (axum::http::StatusCode::NOT_FOUND, Json(serde_json::json!({"error": true, "message": "Task not found or unauthorized access."}))).into_response()
}
...
pub async fn create_task_handler(
    ...
    (axum::http::StatusCode::UNAUTHORIZED, Json(serde_json::json!({"error": true, "message": "Unauthorized or Project Not Found."}))).into_response()
}
