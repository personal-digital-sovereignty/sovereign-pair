...
async def opencode_chat_completions(
    ...
    """
    OpenAI-Compatible Endpoint Proxy para ferramentas como OpenCode / Cursor / Continue.dev.
    Recebe requests e direciona silenciosamente para os LLMs locais/Oracle configurados.
    """
    from src.api.schemas import OpenAIChatRequest
    
    # Parse body properly if sent as dict bypass
    ...
    # Simple heuristic to force local ollama if the model is local.
    # Otherwise use generic DB config or the provided model dynamically.
    if "llama" in target_model.lower() or "qwen" in target_model.lower() or "mistral" in target_model.lower() or "coder" in target_model.lower():
